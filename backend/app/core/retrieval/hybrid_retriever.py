...
from app.core.agent.runtime import submit_context, timeout_seconds
...
try:
    from app.core.retrieval.planning.query_rewrite import rewrite as _rewrite
except ImportError:
    _rewrite = None
...
def _merge_query_results(
    ...
    futures = {submit_context(_QUERY_POOL, _vector_hits_for_query, kb_id, q, top_k): q for q in queries}
...
def retrieve(
    ...
    if settings.keyword_use_rewritten:
        ...
        try:
            from app.core.retrieval.indexing.term_aliases import term_aliases
    ...
    if settings.rerank_enabled and apply_rerank:
        from app.core.retrieval.ranking.reranker import rerank  # 函数内导入：02 前文件不存在
