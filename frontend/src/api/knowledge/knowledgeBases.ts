...
/** 分页读取文档 chunk 明细。 */
export async function getDocChunks(docId: string, offset = 0, limit = 50): Promise<ChunkList> {
  const resp = await apiFetch(`/api/documents/${docId}/chunks?offset=${offset}&limit=${limit}`);
  if (!resp.ok) throw await parseError(resp);
  return resp.json();
}

export async function updateDocChunk(
  docId: string,
  chunkIndex: number,
  text: string,
  expectedUpdatedAt: string,
): Promise<ChunkItem> {
  const resp = await apiFetch(`/api/documents/${docId}/chunks/${chunkIndex}`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ text, expected_updated_at: expectedUpdatedAt }),
  });
  if (!resp.ok) throw await parseError(resp);
  return resp.json();
}
