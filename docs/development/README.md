...
- [用户、知识库与记忆关系设计](../design/用户-知识库-记忆关系设计.md)：面向实验室招新场景，定义用户身份、知识库授权、会话记忆和用户长期记忆之间的数据归属、权限链路与删除影响。
- [复杂问题拆解与意图定向检索实现方案](../design/复杂问题拆解与意图定向检索实现方案.md)：在现有混合检索之上增加子问题规划、主题定向召回、全局召回和覆盖保护；不依赖尚未实现的树形意图分类器。
...
| | [02-用户权限、知识库授权与记忆隔离](phase4-production/02-用户权限-知识库授权与记忆隔离.md) | 多用户招新场景的专项技术方案 |
